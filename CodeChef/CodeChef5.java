...
public class CodeChef5
{
	...
	static int numRounds = scanner.nextInt();
	...
	{
		List<Integer> p1scores = new ArrayList<Integer>();
		List<Integer> p2scores = new ArrayList<Integer>();
		...
		{
			p1scores.add(i, scanner.nextInt());
			p2scores.add(i, scanner.nextInt());
		
			if(p1scores.get(i) > p2scores.get(i))
			{	
				Integer p1Lead = p1scores.get(i) - p2scores.get(i);
				winners.add(i, "1");
				lead.add(i, p1Lead);
			}
			else
			{
				Integer p2Lead = p2scores.get(i) - p1scores.get(i);
				winners.add(i, "2");
				lead.add(i, p2Lead);
			}
		}
		scanner.close();
		biggestLead = lead.get(0);
		winningPlayer = winners.get(0);
		
		for(int index = 0; index < lead.size(); index ++)
		{
			if(lead.get(index) > biggestLead)
			 {
				biggestLead = lead.get(index);
				winningPlayer = winners.get(index);
			 }
		}
		winningPlayer = winningPlayer + " " + biggestLead;
